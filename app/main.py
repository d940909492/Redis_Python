            ...
            if command == "PING":
                ...
                with GLOBAL_LOCK:
                    ...
                    if stored_item and stored_item[0] == 'list' and stored_item[1]:
                        ...
                        if was_notified:
                            ...
                            if rechecked_item and rechecked_item[0] == 'list' and rechecked_item[1]:
                                the_list = rechecked_item[1]
                                popped_element = the_list.pop(0)
                                response = b"".join([b"*2\r\n", f"${len(key)}\r\n".encode(), key, b"\r\n", f"${len(popped_element)}\r\n".encode(), popped_element, b"\r\n"])
                            else: response = b"$-1\r\n"
                        else: response = b"$-1\r\n"
                        if not condition._waiters: del BLOCKING_CONDITIONS[key]
                client_socket.sendall(response)
                ...
                with GLOBAL_LOCK:
                    ...
                    for i in range(0, len(field_value_parts), 2):
                        field, value = field_value_parts[i], field_value_parts[i+1]
                ...
                is_blocking, timeout_ms = False, 0
                try:
                    ...
                    keys, start_ids_str = parts[keys_start_idx:ids_start_idx:2], [p.decode() for p in parts[ids_start_idx::2]]
                ...
                def find_entries(key, start_id):
                    results = []
                    _stored = DATA_STORE.get(key)
                    if _stored and _stored[0] == 'stream':
                        for entry_id_bytes, entry_data in _stored[1]:
                            if parse_id(entry_id_bytes.decode()) > start_id:
                                results.append((entry_id_bytes, entry_data))
                    return results
                
                all_results, resolved_start_ids = {}, {}
                with GLOBAL_LOCK:
                    for i in range(num_keys):
                        key, start_id_val = keys[i], start_ids_str[i]
                        if start_id_val == '$':
                            stored = DATA_STORE.get(key)
                            start_id = parse_id(stored[1][-1][0].decode()) if stored and stored[0] == 'stream' and stored[1] else (0, 0)
                        else:
                            start_id = parse_id(start_id_val)
                        resolved_start_ids[key] = start_id
                        key_results = find_entries(key, start_id)
                        if key_results: all_results[key] = key_results
                ...
                if all_results or not is_blocking:
                    ...
                    with GLOBAL_LOCK:
                        ...
                        if was_notified:
                            for key in keys:
                                key_results = find_entries(key, resolved_start_ids[key])
                                if key_results: all_results[key] = key_results
                        if not condition._waiters: del BLOCKING_CONDITIONS[block_key]
